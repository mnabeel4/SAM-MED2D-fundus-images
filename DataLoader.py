...
class TestingDataset(Dataset):
    ...
    def __init__(self, data_path, image_size=256, mode='train', requires_name=True, point_num=1, return_ori_mask=True,
                 prompt_path=None):
        """
        ...
        json_file = open(os.path.join(data_path, f'label2image_{mode}7.json'), "r")
    ...
    def __getitem__(self, index):
        ...
        assert np.array_equal(ori_np_mask, ori_np_mask.astype(
            float)), f"Mask should only contain binary values 0 and 1. {self.label_paths[index]}"
...
class TrainingDataset(Dataset):
    def __init__(self, data_dir, image_size=256, mode='train', requires_name=False, point_num=1, mask_num=5):
        """
        ...
        dataset = json.load(open(os.path.join(data_dir, f'image2label_{mode}7.json'), "r"))
...
if __name__ == "__main__":
    train_dataset = TrainingDataset("/data-demo/data-demo", image_size=256, mode='train', requires_name=True, point_num=1,
                                    mask_num=5)
    print("Dataset:", len(train_dataset))
    train_batch_sampler = DataLoader(dataset=train_dataset, batch_size=16, shuffle=True, num_workers=6)
