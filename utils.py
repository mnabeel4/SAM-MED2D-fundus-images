...
def get_boxes_from_mask(mask, box_num=1, std=0.1, max_pixel=5):
    ...
    for box in boxes:
        ...
        max_noise = max(1, min(max_pixel, int(noise_std * 5)))  # Ensure max_noise is at least 1
        # Add random noise to each coordinate
        noise_x = np.random.randint(-max_noise, max_noise + 1)  # Adjust the range to include max_noise
        noise_y = np.random.randint(-max_noise, max_noise + 1)
